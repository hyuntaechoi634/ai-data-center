...
#' @details gcamdata-ai PATCH: additionally subtracts AI and conventional data-centre scope-1 water
#'   (L1280.water_km3_R) from the
#'   aggregate industrial water, mirroring the existing paper/food subtractions, so the "other industry"
#'   water coefficient excludes the new compute sectors. Their water_td_ind demand restores the carve,
#'   keeping the base-year water_td_ind market balanced when the compute sectors are added.
#' @importFrom assertthat assert_that
#' @importFrom dplyr arrange distinct filter if_else group_by left_join mutate select
#' @importFrom tidyr complete nesting
#' @author GPK June 2018; gcamdata-ai 2026 (AI water carve)
module_water_L232.water_demand_manufacturing <- function(command, ...) {
  ...
  MODULE_INPUTS <-
    c(FILE = "common/GCAM_region_names",
      ...
      "L1280.water_km3_R")          # gcamdata-ai: data-centre scope-1 water carve
  ...
  if(command == driver.DECLARE_INPUTS) {
    ...
    stub.technology <- subs.share.weight <- tech.share.weight <- share.weight.year <-
      ...
      water_sector <- ai_water <- NULL  # silence package check notes
    ...
    # gcamdata-ai PATCH: subtract compute-sector scope-1 water from the remaining (other-industry) water, the
    # SAME way paper and food are subtracted above. The carve (L1280.water_km3_R) is 2021-only; other base
    # years get 0 via left_join + replace_na. Clamp so the carve never drives the residual negative.
    L1328.water_km3_R_ind_Yh_temp %>%
      select(GCAM_region_ID, year, water_type, water_km3) %>%
      left_join(L1280.water_km3_R %>% rename(ai_water = water_km3),
                by = c("GCAM_region_ID", "year", "water_type")) %>%
      mutate(ai_water = replace_na(ai_water, 0),
             ai_water = if_else(ai_water > water_km3, water_km3, ai_water),
             water_km3 = water_km3 - ai_water) %>%
      select(GCAM_region_ID, year, water_type, water_km3) ->
    ...
    L232.water_km3_R_ind_Yh %>%
      ...
      add_comments("with food processing, paper industry, and AI-compute water use subtracted") %>%
